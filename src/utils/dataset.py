...
def set_seed(seed=42):
    ...
    Bắt buộc gọi hàm này ở đầu mỗi file train.
    """
    random.seed(seed)
    np.random.seed(seed)
    ...
    print(f"[SEED] Da khoa cung Random Seed = {seed}")


class EEGDataset(Dataset):
    """
    Biến mảng NumPy thành định dạng PyTorch Dataset.
    Giữ nguyên Shape gốc (Batch, Kênh, Thời gian) để dùng chung cho cả nhóm.
    """
...
def get_dataloaders(X_train_scaled, X_test_scaled, y_train, y_test, batch_size=64):
    """
    Hàm Factory: Nhận vào dữ liệu ĐÃ CHUẨN HÓA và trả về các DataLoader.
    """
