...
def _to_numpy_labels(tensor_or_array):
    """
    Hàm bổ trợ (Private): Chuyển đổi an toàn từ PyTorch Tensor (kể cả trên GPU) sang NumPy,
    đồng thời tự động lấy argmax nếu đầu vào là xác suất/logits của Model.
    """
    # 1. Chuyển từ PyTorch Tensor về NumPy an toàn
    if isinstance(tensor_or_array, torch.Tensor):
        tensor_or_array = tensor_or_array.detach().cpu().numpy()
    else:
        tensor_or_array = np.array(tensor_or_array)
        
    # 2. Nếu là ma trận xác suất (Batch, Num_Classes), tự động chuyển thành nhãn đơn (0 hoặc 1)
...
def evaluate_metrics(y_true, y_pred):
    ...
    Accepts: List, NumPy Array, hoặc PyTorch Tensors (mô hình dự đoán thô).
    """
    ...
    # average='macro' là bắt buộc để công bằng với lớp thiểu số khi data lệch nghiêm trọng
...
def plot_confusion_matrix(y_true, y_pred, classes=['Low', 'High'], 
    ...
    """Vẽ biểu đồ Ma trận nhầm lẫn (Confusion Matrix) dạng Heatmap sắc nét."""
    y_true = _to_numpy_labels(y_true)
    y_pred = _to_numpy_labels(y_pred)
    ...
    if save_path:
        # Sửa lỗi: Chỉ tạo thư mục nếu đường dẫn có chứa thư mục cha
...
def plot_learning_curves(train_losses, val_losses, train_accs, val_accs, 
    ...
    """
    Vẽ đồ thị song song: Tiến trình hội tụ Loss và Tăng trưởng Accuracy qua các Epochs.
    """
    # Ép kiểu an toàn phòng trường hợp mảng chứa các PyTorch Tensor đơn lẻ
